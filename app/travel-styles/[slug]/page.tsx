...
export async function generateMetadata({ params }: Props) {
  ...
  if (!style) return {};
  return { title: `${style.name} Morocco Tours | DesertBrise Travel`, description: style.description };
}
...
export default async function TravelStyleDetailPage({ params }: Props) {
  ...
  const tours = await getServices({ q: style.query, per_page: 9 }).catch(() => null);
  ...
  return (
    <>
      <section className="relative isolate min-h-[78vh] overflow-hidden bg-[#1d130c] px-5 py-28 text-white sm:px-8 lg:px-10">
        <img src={style.image} alt={style.name} className="absolute inset-0 -z-20 h-full w-full object-cover opacity-70" />
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <h1 className="display-font mt-5 max-w-5xl text-6xl font-semibold leading-[.92] tracking-[-.055em] md:text-8xl">{style.title}</h1>
          <p className="mt-7 max-w-3xl text-lg leading-9 text-white/84">{style.description}</p>
          <div className="mt-8 flex flex-wrap gap-3">
            {style.highlights.map((item) => <span key={item} className="rounded-full border border-white/18 bg-white/12 px-4 py-2 text-sm font-bold text-white/88 backdrop-blur">{item}</span>)}
          </div>
        </div>
      </section>
      ...
      <section className="bg-[#fffaf2] px-5 py-20 sm:px-8 lg:px-10">
        <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[.9fr_1.1fr]">
          <div className="premium-card rounded-[2rem] p-8">
            ...
            <h2 className="display-font mt-3 text-5xl font-semibold leading-[1] tracking-[-.04em] text-[#2b1b11]">{style.name}</h2>
            <p className="mt-5 text-lg leading-9 text-[#75675d]">{style.feeling}</p>
          </div>
          <div className="premium-card rounded-[2rem] p-8">
            <p className="premium-eyebrow">Best for</p>
            <p className="mt-5 text-lg leading-9 text-[#75675d]">{style.bestFor}</p>
          </div>
        </div>
      </section>
      ...
      <section className="bg-[#f9f2e7] px-5 py-24 sm:px-8 lg:px-10">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-3 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <h2 className="display-font max-w-3xl text-5xl font-semibold leading-[1] tracking-[-.04em] text-[#2b1b11] md:text-6xl">Tours for {style.name.toLowerCase()}.</h2>
      ...
      <CTA eyebrow="Design this style for you" title={`Want a ${style.name.toLowerCase()} journey?`} text="Send us your dates and comfort level. We will shape the route around your pace and feeling." />
    </>
  );
}
